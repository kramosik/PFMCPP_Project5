/*
 ...
 The 'this' keyword.
 The purpose of this task is to demonstrate the difference between using member variables in member
functions, vs. using member variables in free functions.
...
1) write 5 UDTs.
    ...
    b) the previous tasks required you to write std::cout statements in main() that accessed the
member variables, or printed out the results from calling member functions. write 2 std::cout
statements in main() that do this for each UDT.

 2) For any std::cout statements you had in main() that accessed member variables of your types or
printed out the results of your member function calls, a) write a member function that prints the
same thing out, but uses the proper techniques inside the member functions to access the same member
variables/functions. b) be explicit with your use of 'this->' in those member functions so we see
how you're accessing/calling those member variables and functions *inside* c) call those member
functions after your std::cout statements in main().  see the example below as to what I'm expecting
you to write.
...
struct MyFoo {
    ...
    void memberFunc()
    {
        std::cout << "MyFoo returnValue(): " << this->returnValue()
                  << " and MyFoo memberVariable: " << this->memberVariable << std::endl;
    } // 2a)
...
int main()
{
    ...
    std::cout << "mf returnValue(): " << mf.returnValue()
              << " and mf memberVariable: " << mf.memberVariable << std::endl; // 1b)
    mf.memberFunc();                                                           // 2c)
    return 0;
}
} // namespace example
...
struct Page {
    ...
    std::string getText() { return chapter + '\n' + content + '\n' + getPageNumber() + '\n'; }
...
struct Screen {
    ...
    bool coordInBound(int x, int y) { return (x > 0 && x < width) && (y > 0 && y < height); }
...
int main()
{
    ...
    std::cout << "page getText(): " << page.getText() << "page pageNumber: " << page.pageNumber << '\n';
    page.memberFunc();
